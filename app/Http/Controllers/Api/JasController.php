    ...
    public function getSummaries(?string $level = '')
{
    ...
    public function get(?int $id = 0)
    {
        if ($id > 0) {
            ...
            ])->leftJoin('users','jas_profiles.technician','users.id')->get();
        }

        $data = array();
        foreach ($jas as $j) {
            $jArray = $j->toArray();
            $jArray['image'] = empty($j->image) ? null : asset($j->image);
            $data[] = $jArray;
        }
    ...
    public function getJasSummaryReportPDF() {
        ...
        foreach($profiles->groupBy('area') as $location =>  $locationProfiles) {
            $chunked = $locationProfiles->chunk(15);
            ...
            foreach($chunked as $ch) {
                ...
                $chunks[] = array(
                    'technicians' => $ch->groupBy(function($profile) {
